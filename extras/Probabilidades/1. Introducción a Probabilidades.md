...
Un experimento aleatorio es un proceso o actividad que, al repetirse en condiciones idénticas

## Sigma-álgebra
Para poder formalizar la teoría de probabilidad, necesitamos primero definir una de las estructuras algebraicas bajo las cuales se rigen las probabilidades: las sigma-álgebras.
...
>- $A_n\in\mathcal{F}\,\forall n\in\mathbb{N}\Rightarrow\bigcup_{n\in\mathbb{N}}A_n\in\mathcal{F}$: Si tenemos una secuencia de conjuntos que pertenecen a $\mathcal{F}$, entonces la unión contable de estos conjuntos también pertenece a $\mathcal{F}$.
