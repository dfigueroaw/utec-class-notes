...
Para calcular $P(X=k)$ cuando tenemos una distribución hipergeométrica, en R se utiliza la función `dhyper(x, k, N-k, n)`. En caso se quiera calcular $P(X\leq x)$, se utiliza la función `phyper(x, k, N-k, n)`, al cual se le puede añadir el parámetro `lower.tail=FALSE` para que la función retorne $P(X>k)$.